...
function DraggableQueueItem({ post, index, onDragStart, onDragOver, onDrop, isDragging, dragOverIndex, theme, dark }: any) {
  ...
  return (
    <div draggable onDragStart={(e) => onDragStart(e, index)} onDragOver={(e) => onDragOver(e, index)} onDrop={(e) => onDrop(e, index)}
      style={{ background: theme.card, border: `1.5px solid ${isOver ? BRAND.primary : theme.border}`, borderRadius: 12, padding: "14px 16px", marginBottom: 8, display: "flex", gap: 12, alignItems: "center", opacity: isDragging === index ? 0.45 : 1, transform: isOver ? "scale(1.01)" : "scale(1)", transition: "all 0.15s ease", cursor: "grab", position: "relative" as const }}>
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 2, color: theme.textT, flexShrink: 0 }}>
  ...
  return (
    <div style={{ minHeight: "100vh", background: BRAND.dark, display: "flex", alignItems: "center", justifyContent: "center", padding: "2rem" }}>
      <div style={{ width: "100%", maxWidth: 480 }}>
        ...
        {[["email","Email address","you@company.com","email"],["pass","Password","Min. 8 characters","password"]].map(([k,l,ph,tp]) => (
          <div key={k} style={{ marginBottom: 12 }}>
            <label style={{ fontSize: 12, fontWeight: 600, color: "rgba(255,255,255,0.5)", display: "block", marginBottom: 5 }}>{l}</label>
            <input value={(form as any)[k]} onChange={f(k)} placeholder={ph} type={tp} style={{ width: "100%", padding: "11px 13px", borderRadius: 10, border: "1px solid rgba(255,255,255,0.1)", background: "rgba(255,255,255,0.06)", color: "#fff", fontSize: 14, boxSizing: "border-box" }} />
          </div>
        ))}
...
export default function App() {
  ...
  async function loadWorkspaces() {
    ...
    if (data && data.length > 0) { setWorkspaces(data); setActiveWS(data[0].id); await loadPosts(data[0].id); }
      ...
      const ini = userEmail.slice(0, 2).toUpperCase();
      const { data: ws } = await supabase.from("workspaces").insert({ user_id: userEmail, name: "My Workspace", industry: "General", color: BRAND.primary, avatar: ini }).select().single();
  ...
  async function loadPosts(wsId: string) {
    const { data } = await supabase.from("posts").select("*").eq("workspace_id", wsId).order("created_at", { ascending: false });
  ...
  async function uploadImage(file: File): Promise<string | null> {
    ...
    try {
      const fn = `${userEmail}/${Date.now()}-${file.name}`;
      const { error } = await supabase.storage.from("post-images").upload(fn, file, { upsert: true });
      if (error) { alert("Image upload failed: " + error.message); setUploadingImage(false); return null; }
      const { data: u } = supabase.storage.from("post-images").getPublicUrl(fn);
      setUploadingImage(false); return u.publicUrl;
    } catch (err: any) { alert("Upload failed"); setUploadingImage(false); return null; }
  }
  ...
  async function uploadVideo(file: File): Promise<string | null> {
    ...
    try {
      const fn = `${userEmail}/videos/${Date.now()}-${file.name}`;
      const { error } = await supabase.storage.from("post-images").upload(fn, file, { upsert: true });
      if (error) { alert("Video upload failed: " + error.message); setUploadingVideo(false); return null; }
      const { data: u } = supabase.storage.from("post-images").getPublicUrl(fn);
      setUploadingVideo(false); return u.publicUrl;
    } catch (err: any) { alert("Upload failed"); setUploadingVideo(false); return null; }
  }
  ...
  async function publishToYouTube(post: any) {
    ...
    try {
      const res = await fetch("/api/youtube/upload", { method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ title: post.content.slice(0, 100), description: post.content, privacyStatus: "public", videoUrl: post.video_url }) });
      const data = await res.json();
      if (data.success) {
        await supabase.from("posts").update({ status: "published", youtube_id: data.videoId }).eq("id", post.id);
        if (activeWS) setPosts(prev => ({ ...prev, [activeWS]: prev[activeWS].map(p => p.id === post.id ? { ...p, status: "published", youtube_id: data.videoId } : p) }));
  ...
  async function publishNow() {
    ...
    if (data) {
      setPosts(prev => ({ ...prev, [activeWS!]: [data, ...(prev[activeWS!] || [])] }));
      if (draft.platforms.includes("youtube") && draft.video_url) {
        const vid = draft.video_url; const cnt = draft.content;
        resetDraft();
        await publishToYouTube({ ...data, video_url: vid, content: cnt });
        return;
      }
    }
    resetDraft();
  ...
  async function addWorkspace() {
    ...
    const ini = newWS.name.split(" ").map((w: string) => w[0]).join("").slice(0, 2).toUpperCase();
  ...
  async function approvePost(id: string, decision: string) {
    ...
    if (activeWS) setPosts(prev => ({ ...prev, [activeWS]: prev[activeWS].map(p => p.id === id ? { ...p, approval: decision } : p) }));
  }

  async function deletePost(id: string) {
    await supabase.from("posts").delete().eq("id", id);
    if (activeWS) setPosts(prev => ({ ...prev, [activeWS]: prev[activeWS].filter(p => p.id !== id) }));
  }

  function handleDragStart(e: React.DragEvent, idx: number) { setDragIndex(idx); e.dataTransfer.effectAllowed = "move"; }
  ...
  async function handleDrop(e: React.DragEvent, toIdx: number) {
    e.preventDefault();
    if (dragIndex === null || dragIndex === toIdx || !activeWS) { setDragIndex(null); setDragOverIdx(null); return; }
    ...
    const updatedPosts = reordered.map((post: any, i: number) => {
      ...
      if (post.scheduled_date !== slot.date || post.scheduled_time !== slot.time) updates.push(supabase.from("posts").update({ scheduled_date: slot.date, scheduled_time: slot.time }).eq("id", post.id));
    ...
    await Promise.all(updates); setDragIndex(null); setDragOverIdx(null);
  }
    ...
    <div style={{ display: "flex", minHeight: "100vh", fontFamily: "'Segoe UI',system-ui,sans-serif", background: theme.bg, transition: "background 0.3s" }}>
      ...
      <div style={{ width: sidebarOpen ? 240 : 64, flexShrink: 0, background: BRAND.sidebar, display: "flex", flexDirection: "column", transition: "width 0.2s", overflow: "hidden", borderRight: "1px solid rgba(255,255,255,0.05)" }}>
        ...
        <div style={{ padding: "10px 8px", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
          ...
          {workspaces.map(w => { const wP = (posts[w.id] || []).filter((p: any) => p.approval === "pending" && p.status !== "draft").length; const active = w.id === activeWS; return (
            <button key={w.id} onClick={() => switchWorkspace(w.id)} style={{ width: "100%", display: "flex", alignItems: "center", gap: 8, padding: "7px 6px", borderRadius: 9, marginBottom: 2, background: active ? "rgba(124,58,237,0.25)" : "transparent", border: active ? "1px solid rgba(124,58,237,0.35)" : "1px solid transparent", cursor: "pointer" }}>
              ...
              {sidebarOpen && <><div style={{ flex: 1, textAlign: "left", overflow: "hidden" }}><div style={{ fontSize: 12, fontWeight: 600, color: active ? "#fff" : "rgba(255,255,255,0.65)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{w.name}</div><div style={{ fontSize: 10, color: "rgba(255,255,255,0.3)" }}>{w.industry}</div></div>{wP > 0 && <span style={{ background: BRAND.primary, color: "#fff", fontSize: 10, fontWeight: 700, borderRadius: 10, padding: "1px 6px", flexShrink: 0 }}>{wP}</span>}</>}
            </button>); })}
          <button onClick={() => setShowNewWS(true)} style={{ width: "100%", display: "flex", alignItems: "center", gap: 8, padding: "6px 6px", borderRadius: 9, marginTop: 2, background: "transparent", border: "1px dashed rgba(255,255,255,0.1)", cursor: "pointer" }}>
            <div style={{ width: 28, height: 28, borderRadius: "50%", border: "1.5px dashed rgba(255,255,255,0.2)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><span style={{ color: "rgba(255,255,255,0.3)", fontSize: 16 }}>+</span></div>
        ...
        <div style={{ padding: "10px 8px", borderTop: "1px solid rgba(255,255,255,0.06)" }}>
          {sidebarOpen && totalPending > 0 && <div style={{ background: "rgba(124,58,237,0.15)", border: "1px solid rgba(124,58,237,0.25)", borderRadius: 9, padding: "8px 10px", marginBottom: 8 }}><div style={{ fontSize: 11, color: BRAND.primaryL, fontWeight: 600 }}>{totalPending} pending approval{totalPending > 1 ? "s" : ""}</div></div>}
          <button onClick={() => signOut({ callbackUrl: "/" })} style={{ width: "100%", display: "flex", alignItems: "center", gap: 10, padding: "7px 6px", borderRadius: 9, background: "transparent", border: "none", cursor: "pointer" }}>
            ...
            {sidebarOpen && <div style={{ textAlign: "left" }}><div style={{ fontSize: 12, color: "rgba(255,255,255,0.7)", fontWeight: 600 }}>{userName}</div><div style={{ fontSize: 10, color: "rgba(255,255,255,0.3)" }}>{t("Sign out","Uitloggen")}</div></div>}
          </button>
        </div>
      </div>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        <div style={{ background: theme.headerBg, borderBottom: `1px solid ${theme.border}`, padding: "0 24px", display: "flex", alignItems: "center", height: 58, gap: 12, flexShrink: 0, position: "relative", transition: "background 0.3s" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div><div style={{ fontSize: 14, fontWeight: 700, color: theme.text }}>{ws?.name}</div><div style={{ fontSize: 11, color: theme.textT }}>{ws?.industry}</div></div>
          </div>
          <div style={{ flex: 1 }} />
          <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
            ...
            <div style={{ position: "relative" }}>
              ...
              {showLangMenu && <div style={{ position: "absolute", right: 0, top: "calc(100% + 8px)", background: theme.card, border: `1px solid ${theme.border}`, borderRadius: 10, boxShadow: "0 8px 30px rgba(0,0,0,0.18)", overflow: "hidden", zIndex: 200, minWidth: 150 }}>
          ...
          {showSupport && <div style={{ position: "fixed", right: 0, top: 58, width: 360, height: "calc(100vh - 58px)", background: theme.card, borderLeft: `1px solid ${theme.border}`, boxShadow: "-4px 0 20px rgba(0,0,0,0.12)", zIndex: 200, overflowY: "auto" }}>
            ...
            <div style={{ padding: "16px" }}>
              ...
              {[{ q: t("How do I create a post?","Hoe maak ik een post?"), a: t("Click '+ Create Post' in the top right corner.","Klik op '+ Maak Post' rechtsboven.") },{ q: t("How do I connect YouTube?","Hoe koppel ik YouTube?"), a: t("Go to Settings → Connected accounts.","Ga naar Instellingen → Gekoppelde accounts.") },{ q: t("How does approval work?","Hoe werkt goedkeuring?"), a: t("Go to 'Approval' to approve or reject posts.","Ga naar 'Goedkeuring' om posts te beoordelen.") }].map((faq, i) => (
          ...
          {showNews && <div style={{ position: "fixed", right: 0, top: 58, width: 360, height: "calc(100vh - 58px)", background: theme.card, borderLeft: `1px solid ${theme.border}`, boxShadow: "-4px 0 20px rgba(0,0,0,0.12)", zIndex: 200, overflowY: "auto" }}>
            ...
            <div style={{ padding: "16px" }}>
              {[{ date: "March 14, 2026", title: t("YouTube publish","YouTube publiceren"), desc: t("Upload videos directly to YouTube.","Upload video's direct naar YouTube."), tag: "🆕 New" },{ date: "March 14, 2026", title: t("Publish now","Nu publiceren"), desc: t("Publish posts immediately without scheduling.","Publiceer posts direct zonder inplannen."), tag: "🆕 New" },{ date: "March 14, 2026", title: t("Dark mode","Donkere modus"), desc: t("Toggle dark mode from the top bar.","Schakel donkere modus in."), tag: "🌙 New" },{ date: "March 12, 2026", title: t("DropPost launched!","DropPost gelanceerd!"), desc: t("The first version is live!","De eerste versie is live!"), tag: "🚀 Launch" }].map((item, i) => (
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "24px" }}>
          {/* DASHBOARD */}
          {page === "dashboard" && <div>
            <div style={{ marginBottom: 20 }}><h1 style={{ fontSize: 22, fontWeight: 800, color: theme.text, marginBottom: 4 }}>Dashboard</h1><p style={{ fontSize: 13, color: theme.textT }}>{t(`Welcome back, ${userName}`,`Welkom terug, ${userName}`)}</p></div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14, marginBottom: 24 }}>
              {[{ label: t("Total","Totaal"), value: wsPosts.length, color: BRAND.primary, bg: darkMode ? "rgba(124,58,237,0.15)" : BRAND.primaryL, icon: <PenLine size={16} color={BRAND.primary} /> },{ label: t("Scheduled","Gepland"), value: scheduled.length, color: "#0EA5E9", bg: darkMode ? "rgba(14,165,233,0.15)" : "#E0F2FE", icon: <Clock3 size={16} color="#0EA5E9" /> },{ label: t("Published","Gepubliceerd"), value: published.length, color: BRAND.green, bg: darkMode ? "rgba(16,185,129,0.15)" : BRAND.greenL, icon: <CheckCircle size={16} color={BRAND.green} /> },{ label: t("Pending","In afwachting"), value: pending.length, color: BRAND.amber, bg: darkMode ? "rgba(245,158,11,0.15)" : BRAND.amberL, icon: <AlertCircle size={16} color={BRAND.amber} /> }].map(s => (
                <div key={s.label} style={{ background: theme.card, borderRadius: 14, padding: "18px 20px", border: `1px solid ${theme.border}` }}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}><div style={{ fontSize: 11, fontWeight: 700, color: theme.textT, letterSpacing: 0.5 }}>{s.label.toUpperCase()}</div><div style={{ width: 32, height: 32, borderRadius: 9, background: s.bg, display: "flex", alignItems: "center", justifyContent: "center" }}>{s.icon}</div></div><div style={{ fontSize: 34, fontWeight: 900, color: s.color }}>{s.value}</div></div>
              ))}
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1.5fr 1fr", gap: 16 }}>
              <div style={{ background: theme.card, border: `1px solid ${theme.border}`, borderRadius: 14, padding: "20px" }}>
                ...
                {wsPosts.slice(0, 4).map((post: any) => (
                  <div key={post.id} style={{ display: "flex", gap: 12, alignItems: "flex-start", marginBottom: 14, paddingBottom: 14, borderBottom: `1px solid ${theme.border}` }}>
                    <div style={{ width: 38, height: 38, borderRadius: 10, background: darkMode ? "rgba(124,58,237,0.15)" : BRAND.primaryL, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>{post.status === "published" ? <CheckCircle size={18} color={BRAND.green} /> : <Clock3 size={18} color={BRAND.primary} />}</div>
                    <div style={{ flex: 1, overflow: "hidden" }}><div style={{ fontSize: 13, color: theme.text, overflow: "hidden", whiteSpace: "nowrap", textOverflow: "ellipsis", marginBottom: 5 }}>{post.content}</div><div style={{ display: "flex", gap: 5, alignItems: "center", flexWrap: "wrap" }}>{post.platforms?.slice(0, 3).map((pid: string) => <PBadge key={pid} pid={pid} dark={darkMode} />)}<span style={{ fontSize: 11, color: theme.textT }}>{post.scheduled_date}</span></div></div>
                    <StatusTag status={post.status} approval={post.approval} />
                  </div>
                ))}
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                <div style={{ background: theme.card, border: `1px solid ${theme.border}`, borderRadius: 14, padding: "18px" }}>
                  ...
                  {PLATFORMS.map(p => { const count = wsPosts.filter((post: any) => post.platforms?.includes(p.id)).length; const pct = wsPosts.length ? Math.round((count / wsPosts.length) * 100) : 0; return (
                    <div key={p.id} style={{ marginBottom: 10 }}><div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}><span style={{ fontSize: 12, color: theme.text, fontWeight: 500 }}>{p.label}</span><span style={{ fontSize: 12, color: theme.textT }}>{count}</span></div><div style={{ height: 5, background: darkMode ? "rgba(255,255,255,0.08)" : "#F3F4F6", borderRadius: 4 }}><div style={{ height: "100%", width: `${pct}%`, background: p.color, borderRadius: 4, transition: "width 0.3s" }} /></div></div>); })}
                </div>
                <div style={{ background: BRAND.gradBtn, borderRadius: 14, padding: "18px", color: "#fff" }}><div style={{ fontSize: 13, fontWeight: 700, marginBottom: 6 }}>{t("Queue times","Wachtrij tijden")}</div><div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>{QUEUE_TIMES.map(qt => <span key={qt} style={{ background: "rgba(255,255,255,0.15)", borderRadius: 7, padding: "4px 10px", fontSize: 12, fontWeight: 600 }}>{qt}</span>)}</div></div>
              </div>
            </div>
          </div>}
              ...
              <div key={post.id} style={{ background: theme.card, border: `1px solid ${theme.border}`, borderRadius: 14, padding: "16px 18px", marginBottom: 10 }}>
                <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 6, alignItems: "center" }}>{post.platforms?.map((pid: string) => <PBadge key={pid} pid={pid} dark={darkMode} />)}<span style={{ fontSize: 11, color: theme.textT }}>• {post.scheduled_date} at {post.scheduled_time}</span><StatusTag status={post.status} approval={post.approval} /></div>
                  </div>
                  <div style={{ display: "flex", gap: 6, flexShrink: 0, flexDirection: "column", alignItems: "flex-end" }}>
                    <div style={{ display: "flex", gap: 6 }}>
                      {post.platforms?.includes("youtube") && post.video_url && post.status !== "published" && <button onClick={() => publishToYouTube(post)} disabled={publishing === post.id} style={{ fontSize: 12, background: "#FF000015", color: "#FF0000", border: "1px solid #FF000030", borderRadius: 7, padding: "5px 10px", cursor: "pointer", fontWeight: 600, opacity: publishing === post.id ? 0.5 : 1 }}>{publishing === post.id ? "Uploading..." : "▶ YouTube"}</button>}
          ...
          {page === "calendar" && (() => {
            ...
            function nav(dir: number) { const d = new Date(calDate); if (calView === "month") d.setMonth(mo + dir); else if (calView === "week") d.setDate(d.getDate() + dir * 7); else d.setDate(d.getDate() + dir); setCalDate(d); }
            const cbtn = (a: boolean) => ({ padding: "7px 16px", borderRadius: 8, border: "none", background: a ? "#0D9488" : (darkMode ? "rgba(13,148,136,0.15)" : "#E0F2F1"), color: a ? "#fff" : "#0D9488", fontSize: 13, fontWeight: 600, cursor: "pointer" as const });
            ...
            return <div>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 20, flexWrap: "wrap" }}>
                ...
                <button style={{ ...cbtn(false), background: darkMode ? "rgba(13,148,136,0.15)" : "#E0F2F1" }} onClick={() => nav(-1)}>‹</button>
                <button style={{ ...cbtn(false), background: darkMode ? "rgba(13,148,136,0.15)" : "#E0F2F1" }} onClick={() => nav(1)}>›</button>
              </div>
              <div style={{ background: theme.card, borderRadius: 16, border: `1px solid ${theme.border}`, overflow: "hidden" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)" }}>
                  {cells.map((day, i) => { const date = day ? new Date(yr, mo, day) : null; const isToday = date ? date.toDateString() === today.toDateString() : false; const dayPosts = date ? postsForDate(date) : []; return (
                    <div key={i} style={{ minHeight: 90, padding: "6px 4px", borderRight: `1px solid ${theme.border}`, borderBottom: `1px solid ${theme.border}`, background: theme.card }}>
                      ...
                      {dayPosts.slice(0,2).map((post: any) => <div key={post.id} style={{ fontSize: 10, background: darkMode ? "rgba(124,58,237,0.15)" : BRAND.primaryL, color: BRAND.primary, borderRadius: 4, padding: "2px 4px", marginBottom: 2, overflow: "hidden", whiteSpace: "nowrap", textOverflow: "ellipsis" }}>{post.content.slice(0,20)}</div>)}
                    </div>); })}
                </div>
              </div>
            </div>;
          ...
          {page === "queue" && <div>
            ...
            <p style={{ fontSize: 13, color: theme.textT, marginBottom: 20 }}>{t("Drag posts to reorder.","Sleep posts om te herordenen.")}</p>
            <div style={{ background: theme.card, border: `1px solid ${theme.border}`, borderRadius: 14, padding: "20px", marginBottom: 16 }}>
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>{QUEUE_TIMES.map(qt => <div key={qt} style={{ background: darkMode ? "rgba(124,58,237,0.15)" : BRAND.primaryL, border: `1px solid ${BRAND.primary}30`, borderRadius: 10, padding: "10px 18px", textAlign: "center" }}><div style={{ fontSize: 16, fontWeight: 800, color: BRAND.primary }}>{qt}</div><div style={{ fontSize: 11, color: theme.textT, marginTop: 2 }}>{scheduled.find((p: any) => p.scheduled_time?.startsWith(qt.slice(0, 2))) ? "1 post" : t("Empty","Leeg")}</div></div>)}</div>
            </div>
            ...
            <div onDragOver={e => e.preventDefault()}>{scheduled.map((post: any, idx: number) => <DraggableQueueItem key={post.id} post={post} index={idx} onDragStart={handleDragStart} onDragOver={handleDragOver} onDrop={handleDrop} isDragging={dragIndex} dragOverIndex={dragOverIdx} theme={theme} dark={darkMode} />)}</div>
          </div>}

          {/* DRAFTS */}
          {page === "drafts" && <div>
            <h1 style={{ fontSize: 20, fontWeight: 800, color: theme.text, marginBottom: 20 }}>{t("Drafts","Concepten")}</h1>
            {draftPosts.length === 0 ? <div style={{ textAlign: "center", padding: "3rem 0", color: theme.textT }}><div style={{ fontSize: 40, marginBottom: 8 }}>📝</div><div>{t("No drafts","Geen concepten")}</div></div>
              : draftPosts.map((post: any) => <div key={post.id} style={{ background: theme.card, border: `1px solid ${theme.border}`, borderRadius: 12, padding: "14px 16px", marginBottom: 8, display: "flex", gap: 12, alignItems: "center" }}><div style={{ flex: 1 }}><div style={{ fontSize: 13, color: theme.text, marginBottom: 5 }}>{post.content}</div><div style={{ display: "flex", gap: 5 }}>{post.platforms?.map((pid: string) => <PBadge key={pid} pid={pid} dark={darkMode} />)}</div></div><button onClick={() => deletePost(post.id)} style={{ fontSize: 12, background: darkMode ? "rgba(239,68,68,0.15)" : BRAND.redL, color: BRAND.red, border: "none", borderRadius: 7, padding: "5px 10px", cursor: "pointer" }}>{t("Delete","Verwijder")}</button></div>)}
          </div>}
          ...
          {page === "approval" && <div>
            ...
            {pending.length === 0 ? <div style={{ textAlign: "center", padding: "4rem 0" }}><div style={{ fontSize: 52, marginBottom: 12 }}>🎉</div><div style={{ fontSize: 16, fontWeight: 700, color: theme.text }}>{t("All caught up!","Alles bijgewerkt!")}</div></div>
              : pending.map((post: any) => <div key={post.id} style={{ background: theme.card, border: `1.5px solid ${BRAND.amber}50`, borderRadius: 16, padding: "18px", marginBottom: 14 }}>
                <p style={{ margin: "0 0 12px", fontSize: 14, lineHeight: 1.6, color: theme.text }}>{post.content}</p>
                <div style={{ display: "flex", gap: 6, marginBottom: 16 }}>{post.platforms?.map((pid: string) => <PBadge key={pid} pid={pid} dark={darkMode} />)}<span style={{ fontSize: 12, color: theme.textT }}>{post.scheduled_date} · {post.scheduled_time}</span></div>
                <div style={{ display: "flex", gap: 10 }}><button onClick={() => approvePost(post.id, "approved")} style={{ flex: 1, padding: "10px", background: darkMode ? "rgba(16,185,129,0.15)" : BRAND.greenL, color: BRAND.green, border: `1px solid ${BRAND.green}40`, borderRadius: 10, cursor: "pointer", fontSize: 14, fontWeight: 700 }}>✓ {t("Approve","Goedkeuren")}</button><button onClick={() => approvePost(post.id, "rejected")} style={{ flex: 1, padding: "10px", background: darkMode ? "rgba(239,68,68,0.15)" : BRAND.redL, color: BRAND.red, border: `1px solid ${BRAND.red}40`, borderRadius: 10, cursor: "pointer", fontSize: 14, fontWeight: 700 }}>✕ {t("Reject","Afwijzen")}</button></div>
              </div>)}
          </div>}
          ...
          {page === "analytics" && <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 14, marginBottom: 24 }}>
              {[{ label: "Total Reach", value: "12,450", color: BRAND.primary },{ label: "Engagement", value: "4.8%", color: BRAND.green },{ label: "Posts", value: wsPosts.length.toString(), color: "#0EA5E9" }].map(s => <div key={s.label} style={{ background: theme.card, border: `1px solid ${theme.border}`, borderRadius: 14, padding: "18px 20px" }}><div style={{ fontSize: 11, fontWeight: 700, color: theme.textT, marginBottom: 10 }}>{s.label.toUpperCase()}</div><div style={{ fontSize: 30, fontWeight: 900, color: s.color }}>{s.value}</div></div>)}
            </div>
          </div>}

          {/* MEDIA */}
          {page === "media" && <div>
            <h1 style={{ fontSize: 20, fontWeight: 800, color: theme.text, marginBottom: 20 }}>{t("Media Library","Mediabibliotheek")}</h1>
            <div style={{ background: theme.card, border: `2px dashed ${theme.border}`, borderRadius: 14, padding: "40px", textAlign: "center" }}><Upload size={36} color={theme.textT} /><div style={{ fontSize: 14, fontWeight: 600, color: theme.text, marginTop: 12 }}>{t("Drop files here","Sleep bestanden hierheen")}</div></div>
          </div>}
          ...
          {page === "team" && <div>
            ...
            <div style={{ background: theme.card, border: `1px solid ${theme.border}`, borderRadius: 12, padding: "14px 16px", display: "flex", gap: 12, alignItems: "center" }}>
              {session.user?.image ? <img src={session.user.image} style={{ width: 40, height: 40, borderRadius: "50%" }} alt="" /> : <Avatar initials={userInitials} color={BRAND.primary} size={40} />}
              <div style={{ flex: 1 }}><div style={{ fontSize: 14, fontWeight: 700, color: theme.text }}>{userName}</div><div style={{ fontSize: 12, color: theme.textT }}>{userEmail}</div></div>
          ...
          {page === "settings" && <div>
            ...
            <div style={{ background: theme.card, border: `1px solid ${theme.border}`, borderRadius: 14, padding: "18px", marginBottom: 14 }}>
              ...
              {[{ id: "youtube", label: "YouTube", color: "#FF0000", icon: "▶", desc: t("Post videos & Shorts","Video's & Shorts") },{ id: "facebook", label: "Facebook", color: "#1877F2", icon: "f", desc: t("Post to Pages","Facebook Pages") },{ id: "instagram", label: "Instagram", color: "#E1306C", icon: "ig", desc: t("Photos & Reels","Foto's & Reels") },{ id: "tiktok", label: "TikTok", color: "#010101", icon: "tt", desc: t("Post videos","Video's") },{ id: "snapchat", label: "Snapchat", color: "#FFCE00", icon: "sc", desc: "Stories" }].map(p => (
                <div key={p.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 0", borderBottom: `1px solid ${darkMode ? "#2A2A40" : "#F3F4F6"}` }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 12 }}><div style={{ width: 36, height: 36, borderRadius: 9, background: p.color + "15", border: `1px solid ${p.color}30`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, color: p.color, fontWeight: 700 }}>{p.icon}</div><div><div style={{ fontSize: 13, fontWeight: 600, color: theme.text }}>{p.label}</div><div style={{ fontSize: 11, color: theme.textT }}>{p.desc}</div></div></div>
                  <button onClick={() => { if (p.id === "youtube") signIn("google", { callbackUrl: "/" }); }} style={{ fontSize: 12, padding: "6px 14px", borderRadius: 8, border: `1px solid ${p.id === "youtube" ? p.color : theme.border}`, background: p.id === "youtube" ? p.color + "10" : "transparent", color: p.id === "youtube" ? p.color : theme.textT, cursor: p.id === "youtube" ? "pointer" : "default", fontWeight: 600 }}>{p.id === "youtube" ? "Connect →" : t("Coming soon","Binnenkort")}</button>
                </div>
              ))}
            </div>
          </div>}
      ...
      {showNewPost && <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 999, padding: "1rem" }}>
        <div style={{ background: theme.modalBg, borderRadius: 20, padding: "24px", width: "min(560px,100%)", boxSizing: "border-box", maxHeight: "90vh", overflowY: "auto", border: `1px solid ${theme.border}` }}>
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            {draft.image_url
              ...
              : uploadingImage
                ...
                : <label style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8, padding: "24px", borderRadius: 12, border: `2px dashed ${theme.border}`, cursor: "pointer", background: theme.codeBg }} onMouseEnter={e => (e.currentTarget.style.borderColor = BRAND.primary)} onMouseLeave={e => (e.currentTarget.style.borderColor = theme.border)}><div style={{ fontSize: 24 }}>📷</div><span style={{ fontSize: 13, fontWeight: 600, color: theme.text }}>{t("Upload image","Upload afbeelding")}</span><span style={{ fontSize: 11, color: theme.textT }}>PNG, JPG, GIF</span><input type="file" accept="image/*" style={{ display: "none" }} onChange={async e => { const f = e.target.files?.[0]; if (f) { const u = await uploadImage(f); if (u) setDraft(d => ({ ...d, image_url: u })); } }} /></label>}
          </div>
          <div style={{ marginBottom: 14 }}>
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            <textarea value={draft.content} onChange={e => setDraft(d => ({ ...d, content: e.target.value }))} placeholder={t("Write your post...","Schrijf je post...")} style={{ width: "100%", minHeight: 120, boxSizing: "border-box", borderRadius: 10, border: `1px solid ${theme.inputBorder}`, padding: "10px 12px", fontSize: 14, resize: "vertical", fontFamily: "inherit", background: theme.inputBg, color: theme.text }} />
